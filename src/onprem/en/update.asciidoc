...
If you are planning to update {CMK} from version {oldstable} to version {current-major}, you should first read the xref:update_major#[Update to version 2.4.0] article, in which we have compiled the most important issues you should consider before and after such an update.
...
For example, if you want to update from version {oldoldstable} to version {current-major}, first update to the intermediate version {oldstable}.
...
In the following example we are installing the package for Ubuntu 24.04 (Noble Numbat):
...
{c-root} omd setversion {oldstable}p23.cee
{c-root} omd version
OMD - Open Monitoring Distribution Version {oldstable}p23.cre
----
...
mysite           {oldstable}p23.cee
test             {current}.cee      default version
----
...
[{shell},highlight=23]]
----
2025-04-30 13:37:23 - Updating site 'mysite' from version {oldstable}p23.cee to {current}.cee...
...
Completed verifying site configuration. Your site now has version {current}.cee.
...
Below is an example of the complete command for an automated update to version `{current}.cee` of `mysite`:

[{shell}]
----
{c-root} omd stop mysite ; omd -f -V {current}.cee update --conflict=install mysite && omd start mysite
----
...
check-mk-raw-{oldstable}p23-el9-38.x86_64.rpm
check-mk-raw-{oldoldstable}p42-el8-38.x86_64.rpm
----
...
[{shell}]
----
{c-root} rpm -e check-mk-raw-{oldoldstable}p42-el8-38.x86_64.rpm
----
...
ii  check-mk-enterprise-{current}  0.jammy  amd64  Checkmk - Best-in-class infrastructure & application monitoring
ii  check-mk-raw-{oldstable}p23        0.jammy  amd64  Checkmk - Best-in-class infrastructure & application monitoring
ii  check-mk-raw-{oldoldstable}p42        0.jammy  amd64  Checkmk - Best-in-class infrastructure & application monitoring
----
...
[{shell}]
----
{c-root} dpkg --purge check-mk-raw-{oldoldstable}p42
(Reading database ... 567850 files and directories currently installed.)
Removing check-mk-raw-{oldoldstable}p42 (0.jammy) ...
...
----
